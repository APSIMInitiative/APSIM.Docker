...
@curl -s -u %BOB_CREDS% -T %PatchFileNameShort%.xml ftp://apsimdev.apsim.info/APSIMClassicBuilds/

cd %APSIM%
if exist %PatchFileNameShort%.diffs.zip (
	echo Uploading %PatchFileNameShort%.diffs.zip...
	@curl -s -u %BOB_CREDS% -T %PatchFileNameShort%.diffs.zip ftp://apsimdev.apsim.info/APSIMClassicBuilds/
)
...
@curl -s -u %BOB_CREDS% -T %PatchFileNameShort%.binaries.WINDOWS.INTEL.exe ftp://apsimdev.apsim.info/APSIMClassicBuilds/
if errorlevel 1 set err=1

echo Uploading %PatchFileNameShort%.binaries.WINDOWS.X86_64.exe...
@curl -s -u %BOB_CREDS% -T %PatchFileNameShort%.binaries.WINDOWS.X86_64.exe ftp://apsimdev.apsim.info/APSIMClassicBuilds/
if errorlevel 1 set err=1

echo Uploading %PatchFileNameShort%.ApsimSetup.exe...
@curl -s -u %BOB_CREDS% -T ApsimSetup\%PatchFileNameShort%.ApsimSetup.exe ftp://apsimdev.apsim.info/APSIMClassicBuilds/
if errorlevel 1 set err=1
